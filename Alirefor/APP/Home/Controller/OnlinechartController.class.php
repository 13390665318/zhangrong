...
class OnlinechartController extends BaseController
{
    public function index(){
        ...
        for($i=1;$i<=$day;$i++){
            ...
            // 最大值
            $ru=D("periods")->where("time='$startime' and db_id=$db_id")->order("num desc")->limit("0,1")->find();
            if($ru==null){
                $arr[$i]["maxnum"]=0;
            }else{
                $arr[$i]["maxnum"]=$ru["num"];
            }
            $arr[$i]["maxtime"]=$startime." ".$ru["ftime"].":00:00";

            // 最小值
            $rus=D("periods")->where("time='$startime'and db_id=$db_id")->order("num asc")->limit("0,1")->find();
            if($rus==null){
                $arr[$i]["muxnum"]=0;
            }else{
                $arr[$i]["muxnum"]=$rus["num"];
            }
            $arr[$i]["muxtime"]=$startime." ".$rus["ftime"].":00:00";
