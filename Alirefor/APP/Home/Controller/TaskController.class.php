...
class TaskController extends BaseController
{
    public function index(){
       ...
       $this->assign("db_id", $db_id);*/
        ...
        $model=M('t_tasks','',$connection);
        $task=$model->where("rid='$roleid'")->select();
        ...
        foreach ($xml['Tasks']['Task'] as $value){
            $nxml[]=$value['@attributes'];
        }
        ...
        foreach ($tasks as $key=>$value){
                $tasks[$key]['isdel'] = $task[$key]['isdel'];
        }
       // dump($tasks);exit;
        foreach($tasks  as $value){
            if($value['isdel']==0&&$value['TaskClass']==0){
                $newtasks[]=$value;
            }
        }
      //  dump($newtasks);exit;
       $this->assign('newtasks',$newtasks);
