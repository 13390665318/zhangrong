...
class LinetimeController extends BaseController
{
    public function index()
    {
        ...
        if ($db_id != 0) {
            $ru['logoutlog.serverID']=$db_id;
            $ru['loginlog.serverID']=$db_id;
        }
        ...
        if (isset($_GET["stime"]) && isset($_GET["etime"])) {
            ...
            $stime = date("Y-m-d ", strtotime('-2day'));
        ...
        for ($i = 0; $i <= $days; $i++) {
            ...
            //dump($endtime);
            $model = D('Logoutlog');
            $ru['_string']="logoutlog.LogTime>='$stime' and logoutlog.LogTime<='$endtime' and loginlog.LogTime>='$stime' and loginlog.LogTime<='$endtime'";
            $arr1 = D('Logoutlog')->where($ru)->join('left join loginlog on loginlog.role_id=logoutlog.role_id')->order('logout_time desc,login_time desc')->field('Logoutlog.role_id,Logoutlog.TodayOnline_time,logout_time,login_time')->buildSql();
            ...
            $sum=array_sum($onlinetime);
            $onlnum = count($onlinetime);
            $data[$i]['average']=round($sum/$onlnum,2);
            $data[$i]['num1'] = 0;
            $data[$i]['num2'] = 0;
            $data[$i]['num3'] = 0;
            $data[$i]['num4'] = 0;
    ...
    public function index2()
    {
        ...
        if (isset($_GET["stime"]) && isset($_GET["etime"])) {
            ...
            $stime = date("Y-m-d ", strtotime('-3day'));
            $etime = date("Y-m-d ", time());
        }
        ...
        for ($i = 0; $i <= $days; $i++) {
            ...
            //dump($endtime);
            $model = D('sign');
            $ru['_string'] = "start_time>='$stime' and start_time<='$endtime'";
            $signcount[$i]['time']=date('Y-m-d ', strtotime("-$i day", strtotime($etime)));
            $signcount[$i]['num']=count($model->Distinct(true)->where($ru)->select());
        }
