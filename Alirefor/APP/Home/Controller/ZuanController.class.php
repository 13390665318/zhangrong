...
class ZuanController extends BaseController
{
    public function index()
    {
        ...
        $ru['currency_use'] ="{}";
        $ru['currency_get'] =array('like',array("{\"40\":%","{\"50\":%"));
        ...
        $ru['LogTime']=array(array('gt',$stime),array('lt',$etime));
        $ru['currency_get'] ="{}";
        $ru['currency_use'] =array('like',array("{\"40\":%","{\"50\":%"));

        $model=D('currency');
        $count2=$model->where($ru)->count();

        $Page=new \Think\Page($count2,20);
        $show=$Page->show();
        $data2 = $model->limit($Page->firstRow.','.$Page->listRows)->where($ru)->select();
        foreach ($data2 as $key =>$value){
            $data2[$key]['currency_use']=json_decode($value['currency_use'],1);
            $data2[$key]['currency_left']=json_decode($value['currency_left'],1);
        }
