...
class EquipmentController extends BaseController
{
    public function index()
    {
        ...
        $equip = $model->where("rid='$roleid'and isusing=1")->select();
        foreach ($equip as $value) {
            $goodsid[] = $value['goodsid'];
        }
        //获取策划goodsXML表
        $xml = file_get_contents('public/Xml/Goods.xml');
        $xml = simplexml_load_string($xml);
        $xmljson = json_encode($xml);
        $xml = json_decode($xmljson, 1);
        //dump($xml['Goods']['Item']);exit;
        foreach ($xml['Goods']['Item'] as $value) {
        ...
        foreach ($goodsid as $value) {
            ...
            foreach ($equip as $k=>$v){
                $equip[$k]['Title']=$equipment[$k]['Title'];
            }
           // dump($equip);exit;
