...
class LogModel extends Model
{
    protected $fields=array('LogTime','Operation','db_id','num','time','f_time','start_time','game_user_id','ip',
        ...
        'currency_reason','value','reason','vip','useYuanbao','left_yuanbao','left_free_yuanbao','use_time');
    //在线添加
    public function YuanbaoGainadd($YuanbaoGain){
        $result=D('Yuanbaogain')->addAll($YuanbaoGain);
    }
    public function Usemoneyadd($usemoney){
        $result=D('Usemoney')->addAll($usemoney);
    }
    public function Fashionadd($Fashion){
        $result=D('Fashion')->addAll($Fashion);
    }
    public function YuanbaoUseadd($YuanbaoUse){
        $result=D('Yuanbaouse')->addAll($YuanbaoUse);
    }
