...
<div id="page-wrapper">
    ...
    <div class="row">
        <div class="col-lg-12">
            <h1 class="page-header">支付金额</h1>
        </div>
    </div>
    <div class="panel panel-default">
        <div class="panel-heading">
            支付金额趋势 <div style="clear: both;"></div>
        </div>
        ...
        <!-- <div class="col-lg-10" style="width: 11%">
             <div class="form-group">
                 <select id="maxOption2" class="selectpicker show-menu-arrow form-control" multiple >
                     <?php if(is_array($qudao)): $i = 0; $__LIST__ = $qudao;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vo["name"]); ?>"><?php echo ($vo["name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
                 </select>
             </div>
         </div>-->
...
<script>
    ...
    function idselect() {
        ...
        location.href=www_url+"index.php?m=Home&c=PayMoney&a=index2&bclothes="+bclothes+"&eclothes="+eclothes+"&stime="+stime+"&etime="+etime;
    ...
    var maxN = eval("Math.max(" + stu.toString() + ")")+100000;
    stu=stu.reverse();
    var data = [stu];
    var data_max = maxN; //Y轴最大刻度
    var line_title = ["支付金额","B"]; //曲线名称
    var y_label = "数量"; //Y轴标题
    var x_label = "时间"; //X轴标题
    var x = [<?php echo ($Stime); ?>]; //定义X轴刻度值
   // var title = "这是标题"; //统计图标标题
    j.jqplot.diagram.base("chart1", data, line_title, "支付金额", x, x_label, y_label, data_max, 1);
    // 两分钟刷新
