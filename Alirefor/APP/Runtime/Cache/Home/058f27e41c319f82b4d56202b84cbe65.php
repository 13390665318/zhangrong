                ...
                <div class="row">
                    <div class="col-sm-6">
                        <div class="dataTables_length" id="dataTables-example_length">
                            ...
                            </select>&nbsp&nbsp&nbsp<label>&nbsp&nbsp&nbsp<input class="btn btn-default"
                    ...
                    <div class="col-sm-12 table-responsive">
                        ...
                        <table class="table table-striped  table-bordered table-hover dataTable "
                            ...
                            <tr>
                                ...
                                <td>剩余钻石</td>
                                <td>剩余红钻(绑钻)</td>
                                <td width="6%">上次登出时间</td>
                                <td width="30%">玩家信息</td>
                            </tr>
                            </thead>
                            <tbody id="ad">
                            <?php if(is_array($loglog)): $i = 0; $__LIST__ = $loglog;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$ko): $mod = ($i % 2 );++$i;?><tr>
                                    <td><?php echo ($ko["login_time"]); ?></td>
                                    <td><?php echo ($ko["role_name"]); ?></td>
                                    <td><?php echo ($ko["role_id"]); ?></td>
                                    <td><?php echo ($ko["ip"]); ?></td>
                                    <td><?php echo ($ko["create_time"]); ?></td>
                                    <td><?php echo ($ko["role_ChangeLife"]); ?>重<?php echo ($ko["role_level"]); ?>级</td>
                                    <td><?php echo ($ko["left_Money"]); ?></td>
                                    <td><?php echo ($ko["left_Bind_Money"]); ?></td>
                                    <td><?php echo ($ko["left_yuanbao"]); ?></td>
                                    <td><?php echo ($ko["left_free_yuanbao"]); ?></td>
                                    <td><?php echo ($ko["last_logout_time"]); ?></td>
                                    <td width="30%">角色好友数:<?php echo ($ko["player_message"]["FriendCount"]); ?> 经验值:<?php echo ($ko["player_message"]["CurExp"]); ?> 战力:<?php echo ($ko["player_message"]["CombatForce"]); ?> 在线时长:<?php echo ($ko["player_message"]["TotalOnlineSecs"]); ?>s PK值:<?php echo ($ko["player_message"]["PKPoint"]); ?> 帮贡:<?php echo ($ko["player_message"]["BangGong"]); ?></td>
...
<script>
    function Cuselect() {
        ...
        if(clothes=="...选择游戏服务器..."){
            ...
            location.href = "index.php?m=Home&c=Loglog&a=index&start_time="+start_time+ "&end_time="+end_time+"&game_user_id="+game_user_id+"&game_user_name="+game_user_name+"&db_id="+clothes;
        }
    }
    function exl(){
        var clothes=$("#clothes").val();
        ...
        var game_user_id = $("#game_user_id").val();
        var game_user_name = $("#game_user_name").val();
            location.href="index.php?m=Home&c=Loglog&a=exl&start_time="+start_time+ "&end_time="+end_time+"&game_user_id="+game_user_id+"&game_user_name="+game_user_name+"&db_id="+clothes;
    }
