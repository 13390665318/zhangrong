...
if($_GET){
        ...
        $Log=$San_log->where("time>='$Betime' and time<='$Entime' and type=91000003")->field("dec")->distinct(true)->select();
        $this->assign("Log",$Log);
		...
		if($uname==null){
		echo "<script>alert('用户信息错误，无法查找');location.href='http://106.15.137.174/Ali/Admin/Prop/index'</script>";exit;
		}
                  $con["uid"]=$uname["uid"];
            }else{
