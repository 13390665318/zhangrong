...
class PaySaveController extends BaseController
{
    public function index(){
        ...
        if($type==1){
            //时间
            $arrs=array('日期','付费玩家','2日留存率','3日留存率','4日留存率','5日留存率','6日留存率','7日留存率','15日留存率','30日留存率');
...
//echo $day;exit;
                    for($i=0;$i<=$day;$i++){
                        ...
                        // 付费总用户
                        // 今天 - 昨天  = 今日新增付费用户
                            $adduser1=D("order")->where("time>='$Strtime' and time<='$Endtime'and db_id=$db_id ")->field("uid")->group("uid")->select(); //今日
			 $Endtimes=date('Y-m-d 23:59:59', strtotime ("+$i day", strtotime($Strtime)-1));
                            $adduser2=D("order")->where("time>='$Strtime' and time<='$Endtimes'and db_id=$db_id ")->field("uid")->group("uid")->select();
			 $arr["adduser"]=count($adduser1)-count($adduser2);
                            $addpay= array_diff(array_values(array_column($adduser1, 'uid')),array_values(array_column($adduser2, 'uid')));

			$addpay=array_values($addpay);
			$adduser=array();
                            for($j=0;$j<count($addpay);$j++){
                                $adduser[$j]["uid"]=$addpay[$j];
                            }
                        // 2日留存
                        $day2=$i+1;
                        ...
                        $result2=array_intersect(array_column($adduser,'uid'),array_column($order2, 'game_user_id'));
                        //   var_dump($result2);
                        ...
                        $result3=array_intersect(array_column($adduser, 'uid'),array_column($order3, 'game_user_id'));
                        ...
                        $result4=array_intersect(array_column($adduser, 'uid'),array_column($order4, 'game_user_id'));
                        ...
                        $result5=array_intersect(array_column($adduser, 'uid'),array_column($order5, 'game_user_id'));
                        ...
                        $result6=array_intersect(array_column($adduser, 'uid'),array_column($order6, 'game_user_id'));
                        ...
                        $result7=array_intersect(array_column($adduser, 'uid'),array_column($order7, 'game_user_id'));
                        ...
                        $result15=array_intersect(array_column($adduser, 'uid'),array_column($order15, 'game_user_id'));
                        ...
                        $result30=array_intersect(array_column($adduser, 'uid'),array_column($order30, 'game_user_id'));
