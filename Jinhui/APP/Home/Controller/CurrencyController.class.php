...
if($_GET){
        ...
        $connection=db($game_id,$db_id);
        $San_log = M('San_log','',$connection);
        $Log=$San_log->where("time>='$Betime' and time<='$Entime' and type='$money_type'")->field("dec")->distinct(true)->select();
        $this->assign("Log",$Log);
        $con["type"]=$money_type;
...
//var_dump($_GET);
  $Userbase = M('San_userbase','',$connection);
	...
	if($uname==null){
		echo "<script>alert('用户信息错误，无法查找');location.href='http://106.15.137.174/Ali/Admin/Currency/index'</script>";exit;
		}
                $con["uid"]=$uname["uid"];
            }else{
