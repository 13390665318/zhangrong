...
// 本周
            ...
            $arr[$i]["num"]=M('user')->where("register_time>='$Strtime' and register_time<='$Endtime'")->count();
//echo $arr[$i]["num"]." ";
...
// 本周
            ...
            $Strtime2=date('Y-m-d 00:00:00', strtotime ("-6 day", strtotime($Strtime)));
            $Endtime2=date('Y-m-d 23:59:59', strtotime ("+0 day", strtotime($Strtime)));
            $day2=M('user as a')->join("sign as b on a.game_user_id =b.game_user_id ")->where("b.start_time>='$Strtime2' and b.start_time<='$Endtime2' and a.register_time>='$Strtime' and a.register_time<='$Endtime'")->distinct(true)->field('a.game_user_id')->select();
            //  echo M('user as a','',$connection)->getLastSql()."<br/>";
            $user=$arr[$i]["num"]-count($day2);
            $data[0]["week"]= $data[0]["week"]+$user;
// 上周
            ...
            $Strtime2s=date('Y-m-d 00:00:00', strtotime ("-13 day", strtotime($Strtimes)));
            $Endtime2s=date('Y-m-d 23:59:59', strtotime ("-7 day", strtotime($Strtimes)));
            $day3=M('user as a')->join("sign as b on a.game_user_id =b.game_user_id ")->where("b.start_time>='$Strtime2s' and b.start_time<='$Endtime2s' and a.register_time>='$Strtimes' and a.register_time<='$Endtimes'")->distinct(true)->field('a.game_user_id')->select();
            //  echo M('user as a','',$connection)->getLastSql()."<br/>";
            $user2=$arr[$i]["num2"]-count($day3);
            $data[1]["week"]= $data[1]["week"]+$user2;
...
//上月本周
            ...
            $Strtime2ss=date('Y-m-d 00:00:00', strtotime ("-36 day", strtotime($Strtime)));
            $Endtime2ss=date('Y-m-d 23:59:59', strtotime ("-30 day", strtotime($Strtime)));
            $day3=M('user as a')->join("sign as b on a.game_user_id =b.game_user_id ")->where("b.start_time>='$Strtime2ss' and b.start_time<='$Endtime2ss' and a.register_time>='$Strtimess' and a.register_time<='$Endtimess'")->distinct(true)->field('a.game_user_id')->select();
            //  echo M('user as a','',$connection)->getLastSql()."<br/>";
            $user=$arr[$i]["num3"]-count($day3);
            $data[2]["week"]= $data[2]["week"]+$user;
            //  exit;
