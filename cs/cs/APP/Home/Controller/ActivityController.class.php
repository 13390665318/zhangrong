...
class ActivityController extends BaseController
{
    ...
    {
        /**  $count = D("activity")->count();// 查询满足要求的总记录数
        $Page = new \Think\Page($count, 20);// 实例化分页类 传入总记录数和每页显示的记录数(20)
        $show = $Page->show();// 分页显示输出
        $this->assign("page", $show);// 赋值分页输出**/
        $arr = D("activity")->order("id asc")->group("type")->select();
        for($i=0;$i<count($arr);$i++){
            if($arr[$i]["type"]==1){
                $arr[$i]["type_name"]="单独模版";
                ...
                $arr[$i]["type_name"]="通用模版";
            }
            if($arr[$i]["start"]==0){
                $arr[$i]["btime"]="创角色开启";
                $arr[$i]["etimr"]="";
            }else if(!strpos($arr[$i]["start"],"-")){
                $arr[$i]["btime"]="开服第".$arr[$i]["start"]."天开启";
                $arr[$i]["etimr"]="";
            }else{
                $arr[$i]["btime"]=$arr[$i]["start"];
                $arr[$i]["etime"]=date("Y-m-d H:i:s",strtotime($arr[$i]["start"])+$arr[$i]["continued"]);
            }
        }
        $this->assign("arr", $arr);
        $this->display();
