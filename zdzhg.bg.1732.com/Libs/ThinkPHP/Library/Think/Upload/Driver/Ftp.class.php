...
//        $getback = ftp_nlist($this->link,  $this->rootPath);
//        $this->error = '文件上传保存错误'.$getback;
//        return false;
        /* 移动文件 */
        if (!ftp_put($this->link, $filename, $file['tmp_name'], FTP_BINARY,0)) {
            $this->error = '文件上传保存错误'.$filename."aaa".$file['tmp_name'];
            return false;
        }
        return true;
    }
