...
class IndexController extends CommonController {
	...
	public function login(){
		if (IS_POST){
			...
			$username = I('post.username', '', 'trim') ? I('post.username', '', 'trim') : $this->error(L("login_username_isnull"), HTTP_REFERER);
			$password = I('post.password', '', 'trim') ? I('post.password', '', 'trim') : $this->error(L("login_password_isnull"), HTTP_REFERER);

			//验证码判断
			$code = I('post.code', '', 'trim') ? I('post.code', '', 'trim') : $this->error(L("login_code_isnull"), HTTP_REFERER);
			if(!check_verify($code, 'admin')) $this->error(L("login_code_failure"), HTTP_REFERER);
			if($admin_db->login($username, $password)){
                
                if(!empty($_POST["saveaccountcookie"]))
                {
                    $_POST["saveaccountcookie"]=="on"?cookie('loginaccount',$username,3600*24*30):cookie('loginaccount',null);
                }
                else
                {
                    cookie('loginaccount',null);
                }
                $_SESSION['username']=$username;
                $_SESSION['password']=$password;
				$this->success(L("public_logon_successful"), U('Index/index'));
            ...
            $loginaccount=cookie('loginaccount');
			if($loginaccount!=null&&$loginaccount!="")
			{
				$this->assign('ischeckedstate',"checked='checked'");
			}
			else
			{
				$this->assign('ischeckedstate',"");
			}
			$this->assign('loginaccount',$loginaccount);
			$this->display();
		}
	}
	...
	public function logout(){
		...
		$this->success(L("public_quit_successfully"), U('Index/login'));
	}
	...
	public function public_userInfo($info = array()){
		...
		if (IS_POST){
			...
			$state ? $this->success(L("public_successful_modification")) : $this->error(L("public_modification_failed"));
		}else {
	...
	public function public_userPwd($info = array()){
		...
		if (IS_POST){
			...
			if(password(I('post.old_password'), $info['encrypt']) !== $info['password'] ) $this->error(L("login_old_password_input_error"));
			if(I('post.new_password')) {
				$state = $admin_db->editPassword($userid, I('post.new_password'));
				if(!$state) $this->error(L("login_failed_to_modify_password"));
			}
			$this->success(L("public_successful_modification"), U('Index/logout'));
		}else {
			$info = $admin_db->where(array('userid'=>$userid))->find();
			$this->assign('info', $info);
	...
	public function public_sessionLife(){
		...
		if(C('LOGIN_ONLY_ONE')){
			...
			if(session_id() != $loginInfo['sessid']){
				$this->error(L("login_other_device_login")."(ip:{$loginInfo['ip']})，".L("login_forced_offline")."！", U('Index/logout'));
			}
			
			if(get_client_ip(0, true) != $loginInfo['ip']){
				$this->error(L("login_login_elsewhere")."(ip:{$loginInfo['ip']})，".L("login_forced_offline")."！", U('Index/logout'));
			}
		}
		//防止cookie超时
		$identity = cookie('identity');
		cookie('identity', $identity);
		$this->success(L("public_normal_login"));
	}
	...
	public function public_clearCatche(){
		...
		if(is_array($list) && !empty($list)){
			foreach ($list as $modelName=>$funcName){
				...
				$this->show(L("public_update_module")."：{$modelName} ...... ".L("public_success")." <br/>");
			}
		}
		$this->show(L("login_cache_update_completed"));
	}
	...
	public function public_sysInfo(){
		...
		if ($sysinfo['sysReShow'] == 'show' && false !== ($strs = @file("/proc/net/dev"))) {
			for ($i = 2; $i < count($strs); $i++) {
				...
				$net_state.="{$info[1][0]} : ".L("public_received")." : <font color=\"#CC0000\"><span id=\"NetInput{$i}\">" . $sysinfo['NetInput' . $i] . "</span></font> GB &nbsp;&nbsp;&nbsp;&nbsp;".L("public_has_been_sent")." : <font color=\"#CC0000\"><span id=\"NetOut{$i}\">" . $sysinfo['NetOut' . $i] . "</span></font> GB <br />";
			}
		}
