...
class SystemController extends CommonController {
		...
		if(IS_POST){
			...
			if($setting_db->where('1')->count()){
				...
				if($res){
					...
					$this->success(L("public_successful_operation"));
				}else{
					$this->error(L("public_operation_failed"));
				}
			}
			$this->success(L("public_successful_operation"));
		}
	...
	public function menu($id = 0, $page = 1, $rows = 10){
		...
		if(IS_POST){
			...
			foreach($list as &$info){
				...
				$info['display']  = $info['display'] ? L("system_display") : '<font color="red">'.L("system_hide").'</font>';
	...
	public function menuAdd(){
		if(IS_POST){
			...
			$res ? $this->success(L("public_return_add_success")) : $this->error(L("public_return_add_errorL"));
		}else{
