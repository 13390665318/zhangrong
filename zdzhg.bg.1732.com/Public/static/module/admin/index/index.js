	...
	var module = {
		...
		event: function(){
			...
			$('.fullscreen-button', '#index-index-layout-north').on('click', function () {
				...
				if(!raw){
					method.messager.tip(language.public_screen_no, 'error');
					return false;
				}

				method.fullscreen.toggle();
				$(this).menu('setText', {target: this, text:( method.fullscreen.status() ? language.public_screen_out : language.public_screen_new)});
		...
		default: function(url){
			...
			setInterval(function(){
				$.post(url, function(res){
					if(!res.status){
					    $.messager.show({ title: language.public_system_hint, msg: res.info, timeout: 3000, showType: 'slide' });
